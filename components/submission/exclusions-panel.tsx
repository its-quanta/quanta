...
import { DEFAULT_EXCLUSION_TEMPLATES } from "@/src/lib/clarifications/constants";
import {
...
export function ExclusionsPanel({
  ...
  const orgTemplates = templates.filter((t) => t.type === "exclusion");
  const templateOptions: TemplateOption[] = [
    ...
    ...DEFAULT_EXCLUSION_TEMPLATES.map((t, index) => ({
      id: `default-${index}`,
      title: t.title,
      description: t.description,
  ...
  return (
    <Card id="exclusions" size="sm">
      <CardHeader className="pb-2">
        <CardTitle className="text-base">Exclusions</CardTitle>
        <CardDescription className="text-xs">
          {items.length} recorded · qualifications excluded from scope
        </CardDescription>
      </CardHeader>
          ...
          <div className="flex flex-col gap-3">
              ...
              {templateOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "rounded-md border border-border p-2 text-left text-xs transition-colors",
            ...
            <div className="space-y-2 border-t border-border pt-3">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="exclusion-title" className="text-xs">
                  Title
                </Label>
                <Input
                  id="exclusion-title"
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="exclusion-desc" className="text-xs">
                  Description
                </Label>
                <Textarea
                  id="exclusion-desc"
