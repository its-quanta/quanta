...
type DocumentsWorkspacePanelProps = {
  projectId: string;
  documents: Document[];
...
export function DocumentsWorkspacePanel({
  projectId,
  documents,
  documentPages,
  onOpenScope,
}: DocumentsWorkspacePanelProps) {
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Upload, classify, and index tender drawings. Build the drawing
            register here — run AI scope discovery from the Scope tab.
          </p>
        </div>
        {onOpenScope ? (
          <Button type="button" variant="outline" size="sm" onClick={onOpenScope}>
