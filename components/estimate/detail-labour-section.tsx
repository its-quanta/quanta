...
import { deleteProjectLabourItemsAction } from "@/src/lib/estimate-generation/actions";
import type { ProjectLabourItem } from "@/src/types/database";

/** No create/update labour line server action exists yet. */
export const LABOUR_LINE_EDITING_ENABLED = false;

type DetailLabourSectionProps = {
  projectId: string;
...
export function DetailLabourSection({
  projectId,
  takeoffItemId,
  labourItems,
  onError,
  ...
  const lines = useMemo(
    () => filterLabourForTakeoff(labourItems, takeoffItemId),
    [labourItems, takeoffItemId]
  );

  const total = useMemo(() => sumLabourCost(lines), [lines]);

  return (
    <div className={cn("space-y-3", className)}>
      {lines.length === 0 ? (
        <p className="text-sm text-muted-foreground">No labour lines generated.</p>
      ) : (
        <ul className="space-y-2">
          {lines.map((line) => {
            const generated = isGeneratedBuildUpLine(line.pricing_source);
            const qtyLabel =
              line.unit === "hr" || line.unit === "hour" ? "Hours" : "Qty";
                ...
                <div className="flex items-start gap-1">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <LineSourceBadge generated={generated} />
                    </div>
                    <dl className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs">
                      <div>
                        <dt className="text-muted-foreground">{qtyLabel}</dt>
                        <dd className="font-mono tabular-nums">
                          {formatQuantity(line.hours)} {line.unit}
                        </dd>
                      </div>
                      <div>
          ...
          <div className="space-y-2 rounded-md border border-dashed border-border p-2.5">
            <div className="grid grid-cols-2 gap-2">
              <Input
                ...
                disabled={!LABOUR_LINE_EDITING_ENABLED}
              />
              <Input
            ...
            <p className="text-xs text-muted-foreground">
              Manual labour editing coming next.
            </p>
          </div>
        ) : null}
      </div>
...
function LineSourceBadge({ generated }: { generated: boolean }) {
  return (
    <Badge
      variant="outline"
      className={cn(
